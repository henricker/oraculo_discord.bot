...
import { ITextChannel } from '@domain/ITextChannel'
...
describe('#UseCase - OnJoinGuildUseCase', () => {
  ...
  it('Should call logger service if anything throws inside exectue method of usecase', async () => {
    ...
    jest
      .spyOn(guildThrowabble.channels.cache, 'find')
  ...
  it('Should send welcome message to textChannel', async () => {
    const { sut } = makeSut()
    jest
      .spyOn(defaultGuild.channels.cache, 'find')
      .mockReturnValueOnce(defaulttextChannel)
