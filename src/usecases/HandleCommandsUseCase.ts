...
export class HandleCommandsUseCase implements IUseCase<Input, Output> {
    ...
    private readonly oraculoTextChannel = process.env.TEXT_CHANNEL_ORACULO as string
    ...
    private async checkCodeMessage(input: Input): Promise<Output> {
        ...
        const code = codeDelimiter.replace(new RegExp(`[\\s${this.codeDelimiter}:]+`, 'g'), '') as CommandCode;
